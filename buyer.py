...
for i in range(22,len(rdn_close)):
    ...
    boll_data.append([time,k3_upper,k3_middle,k3_lowwer,k2_upper,k2_middle,k2_lowwer,k1_upper,k1_middle,k1_lowwer])
...
def job3():
    ...
    :return:
    '''
    logger.info(u"##############4h Boll线向上的BTC交易对##############")
posBollKlinKEY = []
for key in keys:
    if key[-3:] != 'BTC':
        continue
    try:
        ...
        if ret is not None:
            ...
            posBollKlinKEY.append([key,ret])
    except Exception:
        logger.error("Total week klines is less than 3 " + key + " in 1 week line",exc_info=True)
